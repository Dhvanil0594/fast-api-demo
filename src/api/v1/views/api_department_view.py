...
async def read_departments(
    ...
    if department_id is not None:
        ...
        db_department = await _DR.get_department_by_id(db, department_id=department_id)
        if db_department is None:
            raise HTTPException(status_code=404, detail="Department not found")
        return db_department

    # If department_id is not provided, return a list of departments
    return await _DR.get_departments(db=db, skip=skip, limit=limit)
...
async def update_department(
    ...
    db_department = await _DR.update_department(db, department_id, department_data, token)
...
async def delete_department(department_id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):

    if token is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Authorization token is missing")

    # Check if the department exists and delete it
    is_deleted = await _DR.delete_department(db, department_id)
    if not is_deleted:
        raise HTTPException(status_code=404, detail="Department not found")
